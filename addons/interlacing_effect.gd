@tool
class_name InterlacingEffect extends CompositorEffect

const SHADER_PATH: String = "res://addons/interlacing_effect.glsl"
...
@export_range(1, 100, 1.0) var line_width: int = 5
...
func _render_callback(p_effect_callback_type: EffectCallbackType, p_render_data: RenderData) -> void:
	if rd and p_effect_callback_type == EFFECT_CALLBACK_TYPE_POST_TRANSPARENT and pipeline.is_valid():
		...
		if render_scene_buffers:
			...
			if scrolling_lines:
				line_offset = (line_offset + line_width) % 100
